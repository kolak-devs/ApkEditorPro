...
class XmlLayoutResourceFinder(private val context: Context, resDirPath: String?) {
    ...
    private fun findStyleAttributeValue(style: String?, property: XmlLayoutProperties.PropertySpec, visitedStyles: MutableSet<String?>): String? {
        ...
        for (sdk in Build.VERSION.SDK_INT downTo 0) {
            ...
            if (attrs != null) {
                return attrs[property.attrName] ?: return findStyleAttributeValue(styleParents?.get(sdk)?.get(style), property, visitedStyles)
            }
        }
        return null
    }
    ...
    fun findUserDrawable(resName: String?): Drawable? {
        if (!(resourcesDir == null || resName == null || !resName.startsWith("@drawable/"))) {
            ...
            val drawablePngDensity = loadImageFile(File(File(resourcesDir, "drawable-$density"), "$resName2.png"))
            if (drawablePngDensity != null) {
                return drawablePngDensity
            }
            val drawableJpgDensity = loadImageFile(File(File(resourcesDir, "drawable-$density"), "$resName2.jpg"))
            if (drawableJpgDensity != null) {
                return drawableJpgDensity
            }
            val drawableNineDensity = loadNinePatchFile(File(File(resourcesDir, "drawable-$density"), "$resName2.9.png"))
            ...
            for (i in 0 until lenDpi) {
                val drawable = loadImageFile(File(File(resourcesDir, "drawable-" + arrDpi[i]), "$resName2.png"))
                if (drawable != null) {
                    return drawable
                }
            }
            for (i in 0 until lenDpi) {
                val drawable = loadImageFile(File(File(resourcesDir, "drawable-" + arrDpi[i]), "$resName2.jpg"))
                if (drawable != null) {
                    return drawable
                }
            }
            for (i in 0 until lenDpi) {
                val drawable = loadNinePatchFile(File(File(resourcesDir, "drawable-" + arrDpi[i]), "$resName2.9.png"))
                if (drawable != null) {
                    return drawable
                }
            }
                ...
                resourcesDir.listFiles()?.forEach { drawableDir ->
                    if (drawableDir.isDirectory && drawableDir.name.startsWith("drawable")) {
                        drawableDir.listFiles()?.forEach { drawableFile ->
                            ...
                            if (name.lowercase(Locale.getDefault()).endsWith(".png") || name.lowercase(Locale.getDefault()).endsWith(".jpg") || name.lowercase(Locale.getDefault())
                                    .endsWith(".xml")
                            ) {
    ...
    fun getBaseStyle(style: String?): String? {
        ...
        return if (style.startsWith("@style/")) getBaseStyle(style.substring("@style/".length), HashSet()) else style
    }
    ...
    fun addUserDrawable(name: String, data: Intent) {
        try {
            ...
            dir.mkdirs()
            data.data?.let {
                StreamUtilities.transfer(context.contentResolver.openInputStream(it), FileOutputStream(File(dir, "$name.png")))
            }
