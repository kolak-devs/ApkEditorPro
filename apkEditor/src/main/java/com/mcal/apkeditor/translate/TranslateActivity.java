...
/*
 * Inline translation activity, engine is bundled directly into ApkEditor Pro.
 */
public class TranslateActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private class TranslationAdapter extends RecyclerView.Adapter<TranslationAdapter.TranslationViewHolder> {
        ...
        public void onBindViewHolder(@NonNull TranslationViewHolder holder, int position) {
            ...
            holder.btnRetry.setOnClickListener(v -> {
                ...
                showToast(getString(R.string.translate_translating));
    ...
    /*
     * Initializes activity state and starts loading the translation list.
     */
    @Override
    ...
    /*
     * Handles toolbar home button to finish the activity.
     */
    @Override
    ...
    private void showConfigurationDialog() {
        ...
        spinnerService.setAdapter(new ArrayAdapter<>(
                ...
                getResources().getStringArray(R.array.translate_services)));
    ...
    private void loadLanguageLists() {
        googleLangList = parseLanguages(R.array.translate_languages_google);
        yandexLangList = parseLanguages(R.array.translate_languages_yandex);
    }
    ...
    private void prepareAndStartTranslation() {
        this.translatingMessageTextView.setText(R.string.translate_translating);
        this.translatingLayout.setVisibility(View.VISIBLE);
        this.translatedLayout.setVisibility(View.GONE);
        this.actionButton.setText(R.string.translate_stop);
        this.isSaved = false;
        ...
        TranslationEngine.fetchGoogleConfig(
                ...
                new TranslationEngine.ConfigCallback() {
                    ...
                    public void onConfigError(String errorMsg) {
                        showToast(getString(R.string.translate_error));
                        triggerStopProcess();
                    }
                });
    }
    ...
    private void addTranslatedItem(TranslateItem translateItem) {
        ...
        if (translateItem.translatedValue != null && !translateItem.translatedValue.trim().isEmpty()) {
            this.successCount++;
        } else {
            this.failureCount++;
        }

        String string = getString(R.string.translate_translated);
    ...
    private void showCompletionSummary() {
        ...
        String format;
        if (this.totalItemsToTranslate == 0) {
            format = getString(R.string.translate_error_no_string);
        } else {
            format = String.format(Locale.getDefault(), getString(R.string.translate_translated_format), this.successCount);
        }

        if (this.failureCount > 0) {
            format += ", " + String.format(Locale.getDefault(), getString(R.string.translate_failed_format), this.failureCount);
        }

        int untranslatedCount = (this.totalItemsToTranslate - this.successCount) - this.failureCount;
        if (untranslatedCount > 0) {
            format += ", " + String.format(Locale.getDefault(), getString(R.string.translate_untranslated_format), untranslatedCount);
        }
    ...
    private void extractAndSaveTranslations() {
        ...
        for (TranslateItem item : adapterItems) {
            if (item.translatedValue != null && !item.translatedValue.trim().isEmpty()) {
                arrayList.add(new TranslateItem(item.name, item.originValue, item.translatedValue));
            }
        }
