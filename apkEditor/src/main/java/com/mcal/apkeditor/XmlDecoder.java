...
import com.mcal.androlib.FastXmlSerializer;
...
public class XmlDecoder implements ResStreamDecoder {
    ...
    private String getAttributeValue(AXmlResourceParser parser,
            String namespace, String attrName, int index) {
        ...
        if (type == TypedValue.TYPE_ATTRIBUTE) {
            return refDecoder.getAttributeById(data);
            // return String.format("?%s%08X", getPackage(data), data);
        }
        if (type == TypedValue.TYPE_REFERENCE) {
            if (data == 0) {
                // LOGGER.info("id = 0 detected!");
                return "@null";
            }
        ...
        if (type == TypedValue.TYPE_INT_HEX) {
            if ("http://schemas.android.com/apk/res/android"
                    .equals(namespace)) {
                String str = AttrReplacement.getReplacement(attrName, data);
