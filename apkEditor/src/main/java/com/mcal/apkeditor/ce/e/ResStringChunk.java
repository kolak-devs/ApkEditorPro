...
public class ResStringChunk {
    ...
    private static final CharsetDecoder UTF8_DECODER = StandardCharsets.UTF_8
            .newDecoder();
    private final List<Integer> addedStrPositions = new ArrayList<>();
    ...
    private byte[] getStringBytes_utf8(String _str) {
        ...
        byte[] coded = _str.getBytes(StandardCharsets.UTF_8);
    ...
    @NonNull
    private byte[] getStringBytes_utf16(@NonNull String _str) {
        byte[] strBuffer = null;
        byte[] data = _str.getBytes(StandardCharsets.UTF_16LE);
        int offset = 0;
        int length = data.length;
        if (data.length >= 2 && ((data[0] & 0xff) == 255)
                && ((data[1] & 0xff) == 254)) {
