...
public class JKS extends KeyStoreSpi {
    ...
    private static Certificate readCert(@NonNull DataInputStream in) throws IOException, CertificateException {
    ...
    private static void writeCert(@NonNull DataOutputStream dout, @NonNull Certificate cert) throws IOException, CertificateException {
    ...
    @NonNull
    @Contract(pure = true)
    private static byte[] charsToBytes(@NonNull char[] passwd) {
    ...
    public void engineSetKeyEntry(String alias, Key key, char[] passwd, Certificate[] certChain) throws KeyStoreException {
        alias = alias.toLowerCase();
        if (trustedCerts.containsKey(alias)) throw new KeyStoreException("\"" + alias + " is a trusted certificate entry");
