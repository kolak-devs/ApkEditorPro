...
public class InsnGen {
	...
	public void declareVar(CodeWriter code, @NonNull RegisterArg arg) {
	...
	private static String lit(@NonNull LiteralArg arg) {
	...
	public static void makeStaticFieldAccess(CodeWriter code, @NonNull FieldInfo field, @NonNull ClassGen clsGen) {
	...
	private void makeInsnBody(CodeWriter code, @NonNull InsnNode insn, Set<Flags> state) throws CodegenException {
		switch (insn.getType()) {
			case CONST_STR:
	...
	private void filledNewArray(@NonNull FilledNewArrayNode insn, @NonNull CodeWriter code) throws CodegenException {
		code.add("new ");
	...
	private void makeConstructor(@NonNull ConstructorInsn insn, CodeWriter code)
	...
	private void inlineAnonymousConstr(CodeWriter code, @NonNull ClassNode cls, ConstructorInsn insn) throws CodegenException {
	...
	private void makeInvoke(@NonNull InvokeNode insn, CodeWriter code) throws CodegenException {
	 ...
	 */
	private boolean processOverloadedArg(CodeWriter code, @NonNull MethodNode callMth, @NonNull InsnArg arg, int origPos) {
	...
	private boolean inlineMethod(@NonNull MethodNode callMthNode, InvokeNode insn, CodeWriter code) throws CodegenException {
	...
	private void makeTernary(TernaryInsn insn, CodeWriter code, @NonNull Set<Flags> state) throws CodegenException {
		boolean wrap = state.contains(Flags.BODY_ONLY);
		if (wrap) {
			code.add('(');
		}
	...
	private void makeArith(@NonNull ArithNode insn, CodeWriter code, Set<Flags> state) throws CodegenException {
	...
	private void makeArithOneArg(@NonNull ArithNode insn, CodeWriter code) throws CodegenException {
