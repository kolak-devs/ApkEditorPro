package com.termux.terminal.app.manager;
...
public class ToolsManager {
    ...
    private static void extractIdeEnv() {
        final File file = new File(Environment.BIN_DIR, "ideenv");
        if (file.exists()) {
            file.delete();
        }

        String contents = ResourceUtils.readAssets2String(getCommonAsset("ideenv"));
    ...
    private static void extractLogsenderIfNeeded() {
        try {
            final boolean isOld = LOG_SENDER_VERSION > Preferences.getLogSenderVersion();
            if (isOld) {
                ...
                Preferences.setLogSenderVersion(LOG_SENDER_VERSION);
            }
        } catch (IOException e) {
    ...
    private static void extractGradlePlugin() {
        final File repoDir = new File(Environment.ANDROIDIDE_HOME, "repo");
        FileUtils.createOrExistsDir(repoDir);

        final File zip = new File(Environment.TMP_DIR, "gradle-plugin.zip");
        if (zip.exists()) {
