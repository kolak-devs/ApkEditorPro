...
class Downloader private constructor(downloadTask: DownloadTask) : IDownload {
    ...
    class Builder(private val mContext: Context, private var mUrl: String) {
        ...
        fun build(): Downloader {
            mUrl =
                mUrl.ifEmpty { throw MalformedURLException("The entered URL is not valid") }

            val binFolder = File(mContext.filesDir.path + File.separator + "bin")
            if(!binFolder.exists()) {
                binFolder.mkdirs()
            }
            mDownloadDir = binFolder.path
